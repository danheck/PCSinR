...
// [[Rcpp::export]]
arma::vec activation_function(arma::vec activation, arma::vec input,
                              double decay=0.1, double floor=-1.,
                              double ceiling = 1.) {
  vec output =input;
  ...
  // vec bound = ceiling/2*(sign(input)+1) - floor/2*(sign(-input)+1);
  // vec output = (1-decay)*activation + input % (bound + activation % sign(-input));
  return output;
}
...
// [[Rcpp::export]]
arma::vec update_activation(arma::vec activation, arma::mat weights,
                            double decay=0.1, double floor=-1.,
                            double ceiling = 1.){

  // sum of incoming activation
  vec input = weights.t() * activation;

  // update activations of nodes
  vec activation_updated = activation_function(activation, input, decay, floor, ceiling);
...
// [[Rcpp::export]]
bool check_convergence(arma::vec energy, int iter, double stability=10^-6,
  ...
  if(convergence == "floor"){// [[Rcpp::export]]
    int count = sum(floor(1/stability * energy( span(iter-10,iter-1)))   -
  ...
  if(convergence == "sum"){
    vec changes = diff(energy(span(iter-10,iter)));
    return max(abs(changes)) < stability;
  }

  if(convergence == "max"){
    vec diffs = abs(energy(span(iter-10,iter-1)) - energy(iter));
    return all(diffs < stability) ;
  }
  return true;
}
...
bool convergence_floor(arma::vec energy,
  ...
  return sum(floor(1/stability * energy( span(iter-10,iter-1)))   -
...
// [[Rcpp::export]]
Rcpp::List pcs_matrix_cpp(arma::mat weights, arma::vec start, arma::vec reset,
                          double decay=.1, int maxiter = 1000,
                          double stability=10^-6, String convergence= "floor",
                          bool full=false){

  // initialize matrices:
  int dim = weights.n_cols;
  bool converged;
  vec energy = zeros(maxiter);
  vec input,activ = start;
  mat process = zeros(maxiter,dim);
  process.row(0) = start.t();
  uvec reset_idx = find(reset != 0);
  ...
  while(iter < maxiter){
    ...
    // energy at t + 1
    energy(iter) = as_scalar(- activ.t() * weights * activ);
  ...
  return Rcpp::List::create(Rcpp::Named("iterations") = iter,
                            ...
                            Rcpp::Named("activation") = NumericVector(activ.begin(), activ.end()),
